...
module.exports = function(filename, data, options, cb){
  ...
  var connection = this
    , file
    , fileContent = data.stream // check that this is a readable stream;
  ...
  function streamFile(fileWritten){
    ...
    fileContent.on('data', function(chunk){
      var chunkString = chunk.toString('utf8').replace(/\n/g, '');
      SMB2Request('write', {
        'FileId': file.FileId,
      ...
      chunkCount++;
      if( (chunkCount%64) === 0){
        console.log('chunk', chunkCount, 'offset', offset.toNumber(), 'length', chunk.length);
      }
      if(chunkString.indexOf('\0') !== -1){
        console.log('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!nulls in chunk', chunkCount);
